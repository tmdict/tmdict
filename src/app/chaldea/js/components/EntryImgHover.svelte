...
{#if attribute.img.length > 0}
  <a href="https://img.tmdict.com/{attribute.type == 'profile' ? 'servant' : attribute.type}/{currentImg.src}"><img src="https://img.tmdict.com/{attribute.type == 'profile' ? 'servant' : attribute.type}/{currentImg.src}" id={currentImg.id} alt={currentImg.id} /></a>

  {#if attribute.img.length > 1}
    <ul>
      {#each attribute.img as img}
        <li class="image-list" class:active={img.id === currentImg.id}>
          <a class="image-link" href="https://img.tmdict.com/{attribute.type == 'profile' ? 'servant' : attribute.type}/{img.src}" on:mouseover={() => updateImg(img)} on:focus={() => updateImg(img)}>
